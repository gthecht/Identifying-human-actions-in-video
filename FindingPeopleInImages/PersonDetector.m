...
saveStruct = struct('bbox', bbox, 'confTable', confTable, 'existTable', ...
    existTable, 'FrameData', FrameData, 'imgPerm', imgPerm, 'label', label,...
    'labels', labels, 'labelsName', labelsName, 'layers', layers, 'nTest', ...
    nTest, 'options', options, 'outTable', outTable, 'RCNNModl', RCNNModl, ...
    'score', score, 'testOutcome', testOutcome, 'trainTable', trainTable);
saved     = save2Temp(fileName, filePath, saveStruct);
