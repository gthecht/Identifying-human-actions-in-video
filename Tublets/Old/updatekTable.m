...
validPoints     = cell(size(kTable.boxes{ii + 1}, 1),1);
for jj = 1 : size(kTable.boxes{ii + 1}, 1) % changing # of boxes from frame to frame
    ...
    [currFeatures{jj}, validPoints{jj}] = extractFeatures(rgb2gray(kTable.frame{ii + 1}), currboxSurf{jj});
end
kTable.surf{ii + 1} = currboxSurf;
kTable.features{ii + 1} = currFeatures;
kTable.points{ii + 1} = validPoints;
end
